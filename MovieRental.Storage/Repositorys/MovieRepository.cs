...
namespace MovieRental.Storage.Repositorys
{
    public class MovieRepository : BaseRepository<Movie>, IMovieRepository
    {
        ...
        {
            return await GetItemById(id).Include(x => x.Genres).Include(x => x.Directors).Include(x => x.Rating).FirstOrDefaultAsync() ?? throw new NullReferenceException("Movie not found");
        }
    }
}
